...
export const makeMessagesSocket = (config) => {
    ...
    const hasLegacyBizButtonNode = (nodes = [], type) => nodes.some(node => node?.tag === 'biz'
        && Array.isArray(node.content)
        && node.content.some(child => child?.tag === type));
    const withLegacyBizButtonNode = (message, nodes) => {
        const type = getLegacyBizButtonType(message);
        if (!type) return nodes;
        const list = Array.isArray(nodes) ? nodes : [];
        if (hasLegacyBizButtonNode(list, type)) return list;
        return [...list, {
            tag: 'biz',
            attrs: {},
            content: [{ tag: type, attrs: getLegacyBizButtonArgs(message) }]
        }];
    };
